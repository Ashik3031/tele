...
const DEFAULT_COLORS = {
  background: 0xffffff,
  roadColor: 0xe9eef7,
  islandColor: 0xdbe5f3,
  shoulderLines: 0x0b2c73,
  brokenLines: 0x1353cd,
  leftCars: [0x37c6d9, 0x1353cd, 0x018ca5],
  rightCars: [0x0b2c73, 0x018ca5, 0x37c6d9],
  sticks: 0x018ca5,
};
...
export default function Hyperspeed({ effectOptions = DEFAULT_OPTIONS }) {
...
// 3) Apply background AFTER scene exists
...
this.scene.background = new THREE.Color(options.colors.background);
    ...
    safeOptions.distortion =
      distortions[safeOptions.distortion] || distortions.turbulentDistortion;

    // make sure distortion exists
    if (!safeOptions.distortion) {
      safeOptions.distortion = {
    ...
    // ensure distortion uniforms exist
    if (!safeOptions.distortion.uniforms) safeOptions.distortion.uniforms = distortion_uniforms;
    if (!safeOptions.distortion.getDistortion) safeOptions.distortion.getDistortion = distortion_vertex;

    // ---------- init app ----------
    const myApp = new App(container, safeOptions);
  ...
  return <div ref={containerRef} className="hyperspeed-root" />;
}
