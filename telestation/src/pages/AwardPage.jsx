...
export default function AwardsImageCards() {
  const [awards, setAwards] = useState({ employeeOfMonth: [], targetAchieved: [] });
  ...
  const fetchAwards = async () => {
    try {
      ...
      if (response.success && response.data) {
        setAwards({
          employeeOfMonth: response.data.employeeOfMonth || [],
  ...
  if (loading) {
    return (
      <section className="min-h-screen bg-black text-white py-12 sm:py-16 flex items-center justify-center">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-cyan-500"></div>
          <p className="mt-4 text-gray-400">Loading awards...</p>
        </div>
      </section>
    );
  }
        ...
        {name && (
          <div className="absolute bottom-0 left-0 right-0 z-20 p-3 sm:p-4">
            ...
            >
              ...
              {note && (
                <p className="text-xs sm:text-sm text-cyan-300 text-center mt-1">
                  {note}
                </p>
              )}
            </div>
